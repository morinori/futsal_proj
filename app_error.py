...
.css-15zrgzn.e8zbici0,
#MainMenu,
.stDeployButton,
header[data-testid="stHeader"],
.css-1544g2n.e1tzin5v0,
...
@media (max-width: 768px) {
    /* 사이드바를 기본적으로 닫힌 상태로 만들기 */
    .css-1d391kg {  /* Streamlit 사이드바 컨테이너 */
        transform: translateX(-100%);
        transition: transform 0.3s ease;
    }

    /* 사이드바 토글 버튼 개선 */
    .css-14xtw13.e8zbici0 {  /* 사이드바 토글 버튼 */
        z-index: 999;
        position: fixed;
        top: 10px;
        left: 10px;
...
(function() {
    ...
    // 실행 타이밍들
    setTimeout(controlMobileSidebar, 100);
    setTimeout(controlMobileSidebar, 500);
    setTimeout(controlMobileSidebar, 1000);
